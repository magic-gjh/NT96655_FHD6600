...
#endif
SX_TIMER_ITEM(UI_DetLDWSBEEP, UI_DetLDWSBEEP, 3, FALSE)
...
void System_OnInputInit(void)
{
    ...
    //PHASE-2 : Init & Open Lib or LibExt
    {
        ...
        #endif
        SX_TIMER_LDWSBEEP_ID = SxTimer_AddItem(&Timer_UI_DetLDWSBEEP);
#if _MIPS_TODO
        ...
        #endif
        SxTimer_SetFuncActive(SX_TIMER_LDWSBEEP_ID, TRUE);
        #if (TOUCH_FUNCTION == ENABLE)
        //1.設定init值
        GxTouch_Init();
            ...
            if (uiKeyPressCount)
            {
				...
				//MY_ResetAutoLCDOff();
			      System_ResetPowerSaveCount();
                         ...
                         if ((KeyScan_GetPlugDev() != PLUG_HDMI  &&KeyScan_GetPlugDev() != PLUG_TV)&&!Get_AIR_LINKSTATUS())
                            {
                                ...
                                if ( (KeyScan_GetPlugDev() != PLUG_HDMI  &&KeyScan_GetPlugDev() != PLUG_TV)&&!GPIOMap_IsLCDBacklightOn())
        				{
                                ...
                                if(!Getwifi_statue())
                                    {
                                    if(GetAIR_WINDOW_OpenStatus())
                                        Ux_CloseWindow( &UIFlowWndAIR_WaringCtrl, 0);
                                    else    
                                        Ux_OpenWindow( &UIFlowWndAIR_WaringCtrl, 0);
                                    }
                            }
            }
			else if (bPress)
...
static  BOOL  ENABLE_LDWS_BEEP = FALSE; 

void Set_LDWS_BEEP(BOOL Enb)
{
	ENABLE_LDWS_BEEP = Enb;
}
void Get_LDWS_BEEP(void)
{
	return ENABLE_LDWS_BEEP;
}
...
void UI_DetLDWSBEEP(void)
{
    static UINT32 keyBEEPCount = 0;
    static int LastLDWSWarning = 0;
     if(ENABLE_LDWS_BEEP &&  UI_GetData(FL_MOVIE_LDWS)&&gMovData.State==MOV_ST_REC)  
    	{
    	    keyBEEPCount ++;
           if(LastLDWSWarning != GetLDWSDeparture_dirInfo())
            {
                keyBEEPCount = 1;
                LastLDWSWarning = GetLDWSDeparture_dirInfo();
            }
           if(keyBEEPCount < 6)
            {
