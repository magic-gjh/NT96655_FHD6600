...
void UI_DetUSB(void);
extern int SX_TIMER_DET_DELAYOFF_ID;
...
INT32 System_OnUsbInsert(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    SxTimer_SetFuncActive(SX_TIMER_DET_DELAYOFF_ID, FALSE);//jack lan
...
USB_UpdateSource();
    ...
    if(GxCustom_GetControl(GXCUSTOM_CTRL_DELAYOFF_EN) == FALSE)
    {
    //System_PowerOff(SYS_POWEROFF_NORMAL);
    GxCustom_SetControl(GXCUSTOM_CTRL_DELAYOFF_EN, TRUE);			
        
        GxCustom_SetControl(GXCUSTOM_CTRL_DELAYOFF_TIME, 3);
