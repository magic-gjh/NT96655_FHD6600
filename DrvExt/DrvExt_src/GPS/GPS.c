...
#define PRI_GPSRECEIVE            9//11
#define STKSIZE_GPSRECEIVE 6144 // 6144//64KB stack size
...
#define PRI_AIRRECEIVE        10//10
#define STKSIZE_AIRRECEIVE        1024
UINT32 FLG_ID_AIR = 0;
UINT32 AIR_ID = 0;



#define CMD_DATA_LEN  11


static UINT8 CMD_START_LINK[CMD_DATA_LEN] = {0x64 ,0x6A ,0x00, 0x00, 0x00 ,0x04 ,0x00, 0x00 ,0x01 ,0x00 ,0x2D};
static UINT8 CMD_KEEP_LINK[CMD_DATA_LEN] = {0x64 ,0x6A ,0x00, 0x00, 0x00 ,0x04 ,0x00, 0x03 ,0x01 ,0x00 ,0x2A};
...
void GPS_InstallID(void)
{
    ...
    OS_CONFIG_TASK(AIR_ID, PRI_AIRRECEIVE, STKSIZE_AIRRECEIVE, AIRTsk);
    OS_CONFIG_FLAG(FLG_ID_AIR);
  
}


void AIR_InstallID(void)
{
...
static BOOL bAIRRecOpened = FALSE;
ER AIRRec_Open(UART_BAUDRATE BaudRateUart2)
{

    if (bAIRRecOpened)
    return E_SYS;

#if 1//(RECEIVE_FROM_UART2)
    if (uart2_open() == E_OK)
#else
    if (uart_open() == E_OK)
#endif
    {
    debug_msg("\r\nUART2: Open success ,BaudRate=%d\r\n",BaudRateUart2);
#if 1//(RECEIVE_FROM_UART2)
    uart2_init(BaudRateUart2, UART_LEN_L8_S1, UART_PARITY_NONE);
#else
    //uart_init(BaudRateUart1, UART_LEN_L8_S1, UART_PARITY_NONE);
#endif
    ...
    debug_msg("UART1 Start AIR receive task\r\n");
...
#define CMD_HEADER1    0x64
#define CMD_HEADER2    0x6A

#define CMD_FIRSTLINK_RETURN    0x0001
#define CMD_AIRPM_8300    0x8300
#define CMD_FW_INFO_8020    0x8020

static UINT32 PM_VALUE = 0;
static BOOL AIR_LINK_STATUS = FALSE;
static UINT32 AIR_LINK_CNT = 0;

BOOL StartAIRLink(void)
{
    if(!AIR_LINK_STATUS)
    PutHexData(CMD_START_LINK,CMD_DATA_LEN);
}

void Set_AIR_LINKSTATUS(BOOL Enb)
{
    AIR_LINK_STATUS = Enb;
}

BOOL Get_AIR_LINKSTATUS(void)
{
    return AIR_LINK_STATUS;
}

UINT32 Get_PM_Data(void)
{
    return PM_VALUE;
}

UINT32 Get_AIR_LINK_CNT(void)
{
    return AIR_LINK_CNT;
}
...
void AIRTsk(void)
{
    CHAR      DGD_RecSentence[128];
    static UINT32 Step=0;
    static UINT32 dataLEN = 0;
    static UINT32 CMD_Type = 0;
    PutHexData(CMD_START_LINK,CMD_DATA_LEN);
    kent_tsk();
    clr_flg(FLG_ID_AIR, GPS_FLAG_DATAVALID);

    while(1)
    {
    uart2_getChar(DGD_RecSentence); 
    //debug_msg("^R get AIR DATA: 0x%x\r\n",*DGD_RecSentence);
    if(Step == 0)
      	{				   		
		if(*DGD_RecSentence == CMD_HEADER1){
		   Step = 1;				   
		   //debug_msg("Step 1---\r\n");					   
		   }
		   else{
	...
	if(Step == 1)
	{
       	uart2_getChar(DGD_RecSentence); 
        //debug_msg("^R get AIR DATA: 0x%x\r\n",*DGD_RecSentence);
		if(*(DGD_RecSentence) == CMD_HEADER2){
		   //debug_msg("Step 2---\r\n");					   
			AIR_LINK_STATUS = TRUE;
		   Step = 2;}
		   else{
		   Step = 0;}
	}
	...
	if(Step == 2)
	{
       	uart2_getChar(DGD_RecSentence); //data num
       	//debug_msg("^R get AIR DATA: 0x%x\r\n",*DGD_RecSentence);
		   Step = 3;		   	
	}
        ...
        if(Step == 3)
	{
       	uart2_getChar(DGD_RecSentence); //  0  linkdata   1: control data	
       	//debug_msg("^R get AIR DATA: 0x%x\r\n",*DGD_RecSentence);
		   Step = 4;		   	
	}
        if(Step == 4)
	{
       	uart2_getChar(DGD_RecSentence); //  data   len		
       	//debug_msg("^R get AIR DATA: 0x%x\r\n",*DGD_RecSentence);
       	uart2_getChar(DGD_RecSentence+1);
                //debug_msg("^R get AIR DATA: 0x%x\r\n",*(DGD_RecSentence+1));
                dataLEN =( ((*DGD_RecSentence)<<8) | (*(DGD_RecSentence+1)) )&0xffff;
                dataLEN += 1;
                //debug_msg("^G  Step 4 ---   Get Data Len : %d\r\n",dataLEN);	
		   Step = 5;		   	
	}
        if(Step == 5)
	{
	        
       	uart2_getString(DGD_RecSentence,&dataLEN); // 	get last data
       	CMD_Type = (((*DGD_RecSentence)<<8) | (*(DGD_RecSentence+1))) &0xffff;
                //debug_msg("^R CMD_Type   :  0x%04x\r\n",CMD_Type);
                switch(CMD_Type)
                    {
                    case CMD_FIRSTLINK_RETURN:
                    //    debug_msg("^R Link OK? :0x%x  \r\n",*(DGD_RecSentence+2));
                        if(*(DGD_RecSentence+2) == 0)
                            {
                            
                            }
                        break;
                    case CMD_AIRPM_8300:
                        if(*(DGD_RecSentence+3) != 0x03)//IndCode  0x03   fail  else  ok
                            {
                            PM_VALUE = (((*(DGD_RecSentence+4))<<8) | (*(DGD_RecSentence+5))) &0xffff;
                        //    debug_msg("^R  PM value : %d",PM_VALUE);
                            }
                        break;
                    case CMD_FW_INFO_8020:
                        break;
                    }
                if(AIR_LINK_CNT > 3)
                    {
                    PutHexData(CMD_KEEP_LINK,CMD_DATA_LEN);
                    AIR_LINK_CNT = 0;
                    }
                else
                    AIR_LINK_CNT ++;
...
void GPSRecTsk(void)
{
    ...
    while(1)
    {
        ...
        memset(RecSentence,0,strlen(RecSentence));
...
void GPSRec_NMEAParser(CHAR *NMEA_str, UINT32 Len, NMEATYPE NMEAtype)
{
  UINT32 i;
    ...
    case RMC:
        ...
        //debug_msg("\x1b[34m");   //Blue
        //if (Len)
        //{
            //for (i = 0; i < Len; i++)
                //debug_msg("%c",*NMEA_str++);
        //}
        //debug_msg("\r\n");
        //Check processed sentence
        /*
