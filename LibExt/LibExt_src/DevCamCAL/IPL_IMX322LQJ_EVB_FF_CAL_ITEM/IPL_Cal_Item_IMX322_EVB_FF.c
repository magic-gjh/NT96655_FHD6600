...
debug_err((">>>Loffset = %d H = %d\r\n",Bayer.BaseInfo.LineOffset,Bayer.BaseInfo.Height));
    ...
    //for ( fn = 0; fn < 1; fn ++ )
    {
        ...
        FileSys_WriteFile(pFileHdl, Bayer.BaseInfo.PixelAddr, &size, 0, NULL);
        FileSys_CloseFile(pFileHdl);
    }
}
#endif
...
//                        Cal_Status = _CAL_OK;
                ...
                CAL_RW()->fpAddTag(SEC_DP,pTag);
        	       debug_msg("%s........SEC_DP Dark..........fpAddTag=%d......pTag=%d.......\r\n",__func__,CAL_RW()->fpAddTag(SEC_DP,pTag),pTag);	
    ...
    DBG_ERR("\r\n-------------> cal sts(B_OK:0x62 CAL_NONE:0x0CAL_OK:0xfedba950) %x, %x, %x\r\n",pTag->CAL_STATUS, DP_Status_Chk,Cal_Status);

    if (Cal_Status == _CAL_DP_D_OK || Cal_Status == _CAL_OK)
    {
        	debug_msg("%s.Cal_Status.........\r\n",__func__);	
        //write pstore
...
static ER DP_Adjust_Bright(fpUI_cb fpUIcb,UINT32 DmaAddr,UINT32 DmaSize)
{
    ...
    sprintf((char*)m_CalMsg, "Set to Dark :Please Press Shutter Key");
    if(fpUIcb)
        fpUIcb(UICB_WAITKEY,(CHAR*)m_CalMsg,UICOL_GREEN);
...
debug_err((">>>>>DP Br IMX322 status\r\n"));
                ...
                //write tag
                CAL_RW()->fpAddTag(SEC_DP,pTag);
        	       debug_msg("%s.............SEC_DP Bright.....fpAddTag=%d..pTag=%d...........\r\n",__func__,CAL_RW()->fpAddTag(SEC_DP,pTag),pTag);	
        ...
        while(i--)
        {
            if(SensorMode_AE_Check[i][SensorID].SenMode != BYPASS)
            {
                ...
                CAL_RW()->fpAddTag(SEC_AECHK,pTag);
        	       debug_msg("%s..............SEC_AECHK....fpAddTag=%d.............\r\n",__func__, CAL_RW()->fpAddTag(SEC_AECHK,pTag));	
                //release buf
                CalApi()->fpBufEvent()->fpUnInit(buf.Id);
