...
static void IPL_MapSensorMode(IPL_CHGMODE_DATA *ChgMode, IPL_CHGMODE_DATA *PrvMode)
{
    ...
    IPL_AlgSetUIInfo(IPL_SEL_RAW_BITDEPTH_PRV, SEL_RAW_BITDEPTH_12);
    IPL_AlgSetUIInfo(IPL_SEL_RAW_BITDEPTH_CAP, SEL_RAW_BITDEPTH_12);
    switch(ChgMode->Mode)
    {
...
static ER IPL_Cap2Prv(IPL_CHGMODE_DATA *ChgMode, IPL_CHGMODE_DATA *PrvMode)
{
    ...
    //IPL_MapSensorMode(ChgMode, PrvMode);    // update the sensor mode, or the dpc& ecs get tbl error [NTK#RandyHo]<2013/08/25>
    IPL_MapSensorMode(ChgMode, PrvMode);
    ...
    AE_WaitStable(3);//sun 0604
    IPL_CBMsgProc(IPL_CBMSG_PREVIEWSTABLE, NULL);//sun0604
    return E_OK;
}
