#ifndef _IQS_SETTINGFLOW_AR0330M_EVB_FF_C
#define _IQS_SETTINGFLOW_AR0330M_EVB_FF_C
...
static ER IFE_IQparam(IQS_FLOW_MSG MsgID, IFE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    case IQS_MANUAL_TRIG:
        ...
        case IQCB_PREIME_D2D_IFE_QV:
            ...
            subf->pFilter->Bin = IfeBinning;

            //DBG_ERR("ISO_Value = %d, bin = %d\r\n", ISO_Value, subf->pFilter->Bin);
            subf->CGain.R  = 256;
            subf->CGain.Gr = 256;
            subf->CGain.Gb = 256;
            subf->CGain.B  = 256;
        ...
        case IQCB_PREIME_D2D_IPE_QV:
            ...
            switch (IMEffect_Value)
            {
                ...
                case SEL_IMAGEEFFECT_BW:
                    subf->pCctrl->IntOfs = ColorEffectTable_BW.IntOfs;
                    subf->pCctrl->SatOfs = (ColorEffectTable_BW.SatOfs + IQ_Saturation[Saturation_Value]);
                    subf->pCctrl->HueRotateEn = ColorEffectTable_BW.HueRotateEn;
                    subf->pCctrl->Suppress.EdgSel = ESD_OUT;
                    subf->pCctrl->Suppress.VdetDiv = 0x60;
                    subf->pCbCrofs->Cbofs = ColorEffectTable_BW.CbOfs;
                    subf->pCbCrofs->Crofs = ColorEffectTable_BW.CrOfs;
                    subf->pYCCcon->YCon   = ColorEffectTable_BW.YCon;
                    subf->pYCCcon->UVCon  = ColorEffectTable_BW.CCon;
                    break;
                case SEL_IMAGEEFFECT_SEPIA:
                    subf->pCctrl->IntOfs = ColorEffectTable_SEPIA.IntOfs;
                    subf->pCctrl->SatOfs = (ColorEffectTable_SEPIA.SatOfs + IQ_Saturation[Saturation_Value]);
                    subf->pCctrl->HueRotateEn = ColorEffectTable_SEPIA.HueRotateEn;
                    subf->pCctrl->Suppress.EdgSel = ESD_OUT;
                    subf->pCctrl->Suppress.VdetDiv = 0x60;
                    subf->pCbCrofs->Cbofs = ColorEffectTable_SEPIA.CbOfs;
                    subf->pCbCrofs->Crofs = ColorEffectTable_SEPIA.CrOfs;
                    subf->pYCCcon->YCon   = ColorEffectTable_SEPIA.YCon;
                    subf->pYCCcon->UVCon  = ColorEffectTable_SEPIA.CCon;
                    break;
                case SEL_IMAGEEFFECT_VIVID:
                    ...
                    subf->pCctrl->Suppress.EdgSel = ESD_OUT;
                    subf->pCctrl->Suppress.VdetDiv = 0x60;
                    subf->pCbCrofs->Cbofs = ColorEffectTable_VIVID.CbOfs;
                    subf->pCbCrofs->Crofs = ColorEffectTable_VIVID.CrOfs;
                    subf->pYCCcon->YCon   = ColorEffectTable_VIVID.YCon;
                    subf->pYCCcon->UVCon  = ColorEffectTable_VIVID.CCon;
                    break;
...
{
    UINT32 ISO_Value, Zoom_Value, SenMode_Value, IMEffect_Value;
    UINT32 WDR_Value = IPL_AlgGetUIInfo(IPL_SEL_WDR);
...
/*
                    DBG_ERR(".uiGDGain = %d\r\n", GetRtxGainInfo.uiGDGain);
                    DBG_ERR(".uiGLGain = %d\r\n", GetRtxGainInfo.uiGLGain);
                    DBG_ERR(".uiLDGain = %d\r\n", GetRtxGainInfo.uiLDGain);
                    DBG_ERR(".uiLLGain = %d\r\n", GetRtxGainInfo.uiLLGain);
                    DBG_ERR(".uiRGain  = %d\r\n", GetRtxGainInfo.uiRGain );
                    DBG_ERR(".uiGGain  = %d\r\n", GetRtxGainInfo.uiGGain );
                    DBG_ERR(".uiBGain  = %d\r\n", GetRtxGainInfo.uiBGain );
                    DBG_ERR(".uiEGain  = %d\r\n", GetRtxGainInfo.uiEGain );
*/
                    subf->Retinex.pRetinex = &IME_RetInex[WDR_Value];
                    break;
                default:
...
ER IQCB_SettingFlowFunc(IQS_FLOW_MSG MsgID, ENG_SUBFUNC *Info, IPL_HAL_GROUP* group)
{
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PRV_SIE_INIT:
            IQS_SetVerInfo(IQS_AR0330_VerInfo);
            IQS_SetBuildDate(IQS_AR0330_BuildDate);
            break;
...
UINT32 IQS_GetRetinexMinRatio(void)
{
    ...
    UINT32 WDR_Value = IPL_AlgGetUIInfo(IPL_SEL_WDR);
    return IME_RetInex[WDR_Value].SubRatio;
}
